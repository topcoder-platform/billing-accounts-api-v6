...
export class BillingAccountsService {
  ...
  async list(q: QueryBillingAccountsDto) {
    const { clientId, userId, status, page = 1, perPage = 20, sortBy, sortOrder = 'asc' } = q;
    ...
    const [total, items] = await this.prisma.$transaction([
      ...
      this.prisma.billingAccount.findMany({
        ...
        skip: (page - 1) * perPage,
        take: perPage,
        orderBy: sortBy === 'remainingBudget'
    ...
    const lockedMap = new Map(lockedAgg.map(r => [r.billingAccountId, r._sum.amount || 0]));
    const consumedMap = new Map(consumedAgg.map(r => [r.billingAccountId, r._sum.amount || 0]));
    ...
    const data = items.map(i => {
      ...
      return { ...i, lockedBudget: locked, consumedBudget: consumed, totalBudgetRemaining: remaining };
    });
  ...
  async create(dto: CreateBillingAccountDto, createdBy?: string) {
    return this.prisma.billingAccount.create({
      data: {
        ...
        salesTax: dto.salesTax !== undefined ? new Prisma.Decimal(dto.salesTax) : null,
  ...
  async get(billingAccountId: string) {
    ...
    const locked = ba.lockedAmounts.reduce((sum, r) => sum + Number(r.amount), 0);
    const consumed = ba.consumedAmounts.reduce((sum, r) => sum + Number(r.amount), 0);
  ...
  async update(billingAccountId: string, dto: UpdateBillingAccountDto) {
    return this.prisma.billingAccount.update({
      ...
      data: {
        ...
        ...(dto.markup !== undefined ? { markup: new Prisma.Decimal(dto.markup) } : {}),
        ...(dto.clientId !== undefined ? { clientId: dto.clientId } : {}),
        ...(dto.projectId !== undefined ? { projectId: dto.projectId } : {}),
        ...(dto.poNumber !== undefined ? { poNumber: dto.poNumber } : {}),
        ...(dto.subscriptionNumber !== undefined ? { subscriptionNumber: dto.subscriptionNumber } : {}),
        ...(dto.isManualPrize !== undefined ? { isManualPrize: dto.isManualPrize } : {}),
        ...(dto.paymentTerms !== undefined ? { paymentTerms: dto.paymentTerms } : {}),
        ...(dto.salesTax !== undefined ? { salesTax: dto.salesTax !== null ? new Prisma.Decimal(dto.salesTax) : null } : {}),
  ...
  async lockAmount(billingAccountId: string, dto: LockAmountDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const consumed = await tx.consumedAmount.findUnique({
        where: { consumed_unique_challenge: { billingAccountId, challengeId: dto.challengeId } },
      });
      if (consumed) {
        throw new Error('Challenge already consumed against this billing account');
      }

      if (dto.amount === 0) {
        await tx.lockedAmount.deleteMany({ where: { billingAccountId, challengeId: dto.challengeId } });
        return { unlocked: true };
      }

      // upsert lock
      const rec = await tx.lockedAmount.upsert({
        where: { locked_unique_challenge: { billingAccountId, challengeId: dto.challengeId } },
        create: { billingAccountId, challengeId: dto.challengeId, amount: new Prisma.Decimal(dto.amount) },
